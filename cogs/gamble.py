...
class SimpleView(ui.View):
    ...
    async def gifted(self, interaction: Interaction, _: ui.Button):
        ...
        embed = Embed(title="🎁 Top 10 Gifters", description=description, color = Color.from_rgb(200, 200, 0))
        cnx.close()
        cursor.close()
    ...
    async def coin_leaderboard(self, interaction: Interaction, _: ui.Button):
        ...
        for i in coins:
            ...
            if index == 1:
                ...
                description += f"**{index}.** {user.display_name} - `{i[2]} coin{"s" if int(i[2]) != 1 else ""}`\n"
            index += 1
        embed = Embed(title="🏆 Top 10 Leaderboard", description=description, color = Color.from_rgb(255, 25, 0))
        cursor.close()
        cnx.close()
        await interaction.response.edit_message(embed=embed, view=self)
...
class Gamble(commands.Cog):
    ...
    @app_commands.command(
        name="check-balance",
        description="Check the QTC coin balance of a user."
    )
    ...
    async def balance(self, interaction: Interaction, member: Member = None):
        ...
        if coins == []:
            cursor.execute(f'INSERT INTO coins(user, server, coins, gifted) VALUES("{interaction.user.id}", "{interaction.guild.id}", 10, 0)')
            cnx.commit()
            amount = 10
        else:
            amount = coins[0][2]
        ...
        if member == interaction.user:
            await interaction.response.send_message(f'You have {amount} coin{"s" if int(amount) > 1 else ""}.')
        else:
            await interaction.response.send_message(f'{member.mention} has {amount} coin{"s" if int(amount) > 1 else ""}.')
        cursor.close()
    ...
    async def lb(self, interaction: Interaction):
        ...
        embed = Embed(title="🏆 Top 10 Leaderboard", description=description)
    ...
    async def balance(self, interaction: Interaction, member: Member = None):
        ...
        match place2:
            ...
            case 3:
                award2 = " 🥉"
        embed = Embed(
                    title=f"QTC Coin Balance of {member.name}",
